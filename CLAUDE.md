...
- 本リポジトリのテストは **vitest**。ルート `npm test`（全 workspace 実行）。統合テストは `packages/pinecone-client` 限定の `test:integration`（`npm -w @easy-flow/pinecone-client run test:integration`、`PINECONE_INTEGRATION=true` 必須）
...
- `src/tools.ts` — ワークフロー操作ツール群のファクトリ（登録ツール一覧は `index.ts` の `tools` 配列が正本）
