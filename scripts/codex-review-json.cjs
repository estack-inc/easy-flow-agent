...
// reviewer がしばしばゼロ詰めなしの id (`F-1` / `F-42`) を返すため、
// validator が要求する `F-NNN`（3 桁以上ゼロ詰め）へ canonical 化する。
// 桁数が既に 3 桁以上ならそのまま。数値部を持たない / 形式が全く違うものは
// 触らず（null を返さず原文を返し）、validator にそのまま報告させる。
function canonicalizeFindingId(id) {
  if (typeof id !== 'string') return id;
  const m = id.trim().match(/^F-([0-9]+)$/);
  if (!m) return id;
  const digits = m[1];
  if (digits.length >= 3) return `F-${digits}`;
  return `F-${digits.padStart(3, '0')}`;
}
