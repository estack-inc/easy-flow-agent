...
describe("validatorPrompts", () => {
  ...
  it("各プロンプトがシステムプロンプトのヘッダーを含む", () => {
    expect(validatorPrompts.requirements).toContain(
      "# requirements-validator システムプロンプト",
    );
    expect(validatorPrompts.design).toContain(
      "# design-validator システムプロンプト",
    );
    expect(validatorPrompts.task).toContain(
      "# task-validator システムプロンプト",
    );
    expect(validatorPrompts.outputReview).toContain(
